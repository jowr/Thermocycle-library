...
model OnePhaseInc
  "1-D lumped fluid flow model (Real fluid model) - Incompressible fluid"
...
parameter Boolean subcooled = true "Set to true if cell is subcooled" annotation (Dialog(group = "CV"));
  ...
  /* Geometry */
  Modelica.Units.SI.Length ll(start=lstart, min=1) "Length of this segment";
  ...
  /* Balance variables */
  Modelica.Units.SI.MassFlowRate M_dot(start=Mdotnom) "Mass flow at inlet";
...
//    "Change in mass in control volume";
  ...
  Modelica.Units.SI.HeatFlowRate dUdt(start=0)
...
//  Modelica.SIunits.DerDensityByEnthalpy drdh "Fluid state - rho deriv wrt h";
 ...
 // Real drdt "Fluid state - rho deriv wrt time";
  ...
  Modelica.Units.SI.Volume volume=AA*ll "Control volume";
  Modelica.Units.SI.Mass mass=rho*volume "Mass in control volume";
...
//  dMdt = M_dot_a - M_dot_b;
  ...
  dUdt   = (rho*hh*dldt + rho*ll*dhdt - ll*dpdt + h_a*rho_a*der(la) - h_b*rho_b*der(lb)) * AA;
 /* HEAT TRANSFER */
  AU = YY*ll*Unom;
  C_dot = M_dot*Medium.specificHeatCapacityCp(fluidState);
  NTU = AU/C_dot;
  epsilon =  1 - exp(-NTU);

  SS = YY*ll;
if eps_NTU then
q_dot = C_dot*epsilon*(Text-TT*Outlet- TT*Inlet);
else
...
initial equation
  ...
  annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100},{100,100}}),
         graphics={Rectangle(
          ...
          <p><big>Model <b>OnePhaseInc</b> describes the flow of a fluid in <b>single phase</b> through a channel of variable length.
          ...
          <p><big>The model is based on the same structure of the <a href=\"modelica://MovingBoundary.Components.Complete.Cells.OnePhase\">OnePhase</a> model and it is
          further simplified by assuming incompressible fluid flow: 
       <p><big>
       <ul><li> The density of the fluid is considered constant in the control volume. Static mass balance. </ul>
